...
void Hyperdrive::registerConnectOverride() {
  ...
  // Assign a synthetic IPv4 in the reserved 240.0.0.0/4 block so it can never collide with a real
  // host the Worker might legitimately connect to. The low 16 bits identify this binding; start at
  // a random suffix and walk until we find one not already claimed by another Hyperdrive binding.
